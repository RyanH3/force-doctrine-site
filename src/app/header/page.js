...
import { deleteSession, getCookieData } from "@/app/lib/session";
  ...
  useEffect(() => {
    async function setNavButtonText() {
      ...
      if (userID == "Logged out") {
        ...
        if (isHovered) { // Change text to "Logout"
          setButtonText("Logout");
        }
        else { // Put username into a greeting
      ...
      <div className="flex flex-row font-sans text-yellow-400 auto-cols-auto items-center bg-black px-[1vw] py-[2vw] fixed top-0 right-0 left-0">
        ...
        <nav className="flex flex-row-reverse place-items-center fixed right-0 [&>*]:px-[1vw] md:flex flex-row-reverse place-items-center fixed right-0 [&>*]:px-[1vw]">
          <Link href="/login"
            ...
            <button className="text-sm text-black bg-red-500 transition duration-300 ease-in-out hover:bg-yellow-400 active:bg-yellow-500 rounded-md p-[1vw]"          
                    onClick={() => deleteSession()}>{buttonText}</button></Link>
